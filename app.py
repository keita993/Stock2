...
def get_latest_stock_info(ticker_to_fetch):
    ...
    try:
        ...
        # 指標計算に必要な期間のデータを取得 (最低50日分程度)
        hist = stock.history(period="3mo") # 過去3ヶ月分取得

        if hist.empty:
            return {"error": f"銘柄コード '{ticker_to_fetch}' のデータが見つかりませんでした。"}
        ...
        print(f"銘柄情報取得中にエラー発生 ({ticker_to_fetch}): {str(e)}")
        print(traceback.format_exc())
        return {"error": f"銘柄 '{ticker_to_fetch}' の情報取得中にエラーが発生しました。", "ticker": ticker_to_fetch}
...
def get_portfolio_data():
    ...
    try:
        ...
        for ticker_input in tickers:
            # ここで normalize_ticker を呼び出す
            ticker_to_fetch = normalize_ticker(ticker_input)
            stock_info = get_latest_stock_info(ticker_to_fetch)
            if stock_info and 'error' not in stock_info:
                 # 取得成功時は元のticker_inputも保持
                 stock_info['original_ticker'] = ticker_input
                 portfolio_data.append(stock_info)
            elif stock_info and 'error' in stock_info:
                 # エラー情報を追加（元のticker_inputを使用）
                 portfolio_data.append({"ticker": ticker_to_fetch, "original_ticker": ticker_input, "name": f"{ticker_input} (取得失敗)", "latest_price": None, "short_term_expectation": None})
            else:
                 # 予期せぬエラーケース
                 portfolio_data.append({"ticker": ticker_to_fetch, "original_ticker": ticker_input, "name": f"{ticker_input} (不明なエラー)", "latest_price": None, "short_term_expectation": None})
        ...
        print(traceback.format_exc())
        return jsonify({"error": f"ポートフォリオデータ取得中にエラーが発生しました: {str(e)}"}), 500
...
def login():
    try:
        ...
        print(f"ログイン試行 - ユーザー名: {username}, パスワード: {password}")  # デバッグログ
        ...
        print(f"スタックトレース: {traceback.format_exc()}")  # スタックトレースを追加
